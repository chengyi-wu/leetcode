def bfs(s, adj):
    ...
    while len(frontier) > 0:
        ...
        for u in frontier:
            if u in adj:
                for v in adj[u]:
                    if v not in parent and v not in next:
                        next.append(v)
                        parent[v] = u
        frontier = next
        level[i] = next
